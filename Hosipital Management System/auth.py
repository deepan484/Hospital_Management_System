...
def get_completed_appointments_for_patient(patient_id): # To retrive the completed appointments fro the patients
...
def add_prescription(appointment_id, remarks, symptoms, prescribed_medicines):
    """Function to add a prescription and prescribed medicines to the database."""
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        ...
        cursor.execute("""
            INSERT INTO prescriptions (appointment_id, remarks, symptoms, date)
            VALUES (%s, %s, %s, NOW()) RETURNING prescription_id
        """, (appointment_id, remarks, symptoms))
        prescription_id = cursor.fetchone()[0]

        # Insert prescribed medicines
        for medicine_id, dosage, quantity in prescribed_medicines:
            cursor.execute("""
                INSERT INTO prescribed_medicines (prescription_id, medicine_id, dosage, quantity)
                VALUES (%s, %s, %s, %s)
            """, (prescription_id, medicine_id, dosage, quantity))

        # Commit the transaction
        connection.commit()
        return True, "Prescription and medicines added successfully."
...
def get_prescription_by_appointment(appointment_id):
    """Fetch prescription details and prescribed medicines for an appointment."""
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        ...
        cursor.execute("""
            SELECT prescription_id, remarks, symptoms
            FROM prescriptions
            WHERE appointment_id = %s
        """, (appointment_id,))
        ...
        prescription_id, remarks, symptoms = prescription

        # Fetch prescribed medicines
        cursor.execute("""
            SELECT pm.medicine_id, m.medicine_name, pm.dosage, pm.quantity
        ...
        return {
            ...
            'remarks': remarks,
            'symptoms': symptoms,
            'medicines': [{'medicine_name': med[1], 'dosage': med[2], 'quantity': med[3]} for med in medicines]
        }
...
def get_medicines():
    """Fetch the list of available medicines from the database."""
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
